# lite-organizer 详细设计文档 v1.1

> 对应需求:`lite-organizer-requirements.md`(决策已确认)。本文档为接口/数据/流程定稿,实现以此为据。
> v1.1(2026-08-06):按实现代码(`workspace/lite-organizer/`)校验补充——类别自动分类(category_folder/category_rules)、TMDB api_base/proxy、PUID/PGID 降权、模板上下文变量、接口签名与实际实现对齐、测试清单更新。
...
- 环境变量:`LITE_CONFIG`(配置文件路径,默认 `config.yaml`)、`LITE_TOKEN`(覆盖 `server.token`)
...
> **路径一致性**:qB 与 lite-organizer 容器必须挂载相同路径,contentPath/脚本参数原样透传,不做宿主↔容器映射。`scripts/qb-notify.sh` 的 token 经环境变量 `LITE_TOKEN` 或文件 `/config/lite-token` 注入,脚本不硬编码 token。
